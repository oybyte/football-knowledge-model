...
足球比赛的赛程、赔率、盘口变化和历史规则通常分散在不同来源中，数据口径、采集时间和可信程度不一致。仅凭单一盘口或静态规则进行判断，难以回答三个关键问题：当时实际能看到什么数据、哪条规则或特征影响了结论、赛后能否用同一份证据复盘。

本项目要建设的是一套面向研究和分析人员的足球竞猜知识库检索与预测系统。系统将多源比赛数据按时间保存，完成归一化、特征计算、规则检索、冲突仲裁、预测发布和赛果回填，并保留数据来源、时间完整性、规则版本和推理证据。它首先解决分析过程的可追溯和可验证问题，不承诺结果必然正确，也不把原型置信度解释为生产准确率或 ROI。

## 最终验收产品

最终验收对象是一个可本地启动、可联调、可验证的前后端产品闭环，而不是单独的页面原型或架构文档：

- 前端从 `prototype-1.0.0/` 提供赛事浏览、比赛分析、规则库、回测、数据接入和历史相关视图。
- 后端从 `server/` 提供真实 API，负责数据接入、官方与人工数据合并、特征和规则计算、预测及证据持久化。
- SQLite 保存规则、比赛、盘口、特征、预测、回填和审计数据；Redis 可作为缓存、队列、锁和共享限流后端。
- 系统能够展示一场比赛的数据来源、有效时间、命中的规则、特征快照、方向结论和推理链，并在赛后完成结果回填与复盘。
- 产品具备可验收的鉴权、限流、TLS、健康检查、审计和 Docker Compose 运行形态，但线上长期运行和收益能力仍不属于当前版本的完成承诺。

具体验收场景和通过条件见 `docs/current-status.md` 的“最终验收范围”。
